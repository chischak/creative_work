...
void COMMIS::on_Create_vertex_clicked()
{
...
void COMMIS::on_Delete_vertex_clicked()
{
    ui->Route_result->clear();
    ui->Length_route->clear();
    if (num_vertexs != 0)
    {
        ...
        if (del_index == -1)
        {
            QMessageBox::warning(this, "Тут какая-то ошибка!", "Нельзя оставлять поля пустыми!");
        }
        else if (del_index + 1 > num_vertexs)
        {
            QMessageBox::warning(this, "Тут какая-то ошибка!", "А вы уверены, что такая вершина существует?");
        }
        else
        {
...
void COMMIS::on_Clear_graph_clicked()
{
    num_vertexs = 0;
    vertexs.clear();
    edges.clear();
    ...
    ui->Length_route->clear();
    openGlW->updatedraw();
    QMessageBox::warning(this, "", "Тут ничего нет, вы очистили поле!");
}


void COMMIS::on_Create_edge_clicked()
{
    ui->Route_result->clear();
    ui->Length_route->clear();
    if (vertexs.size() > 1)
    {
        ...
        {
            QMessageBox::warning(this, "Тут какая-то ошибка!", "Такой вершины нет... Как так?");
        }
...
void COMMIS::on_Optimal_rout_clicked()
{
    ui->Route_result->clear();
    ui->Length_route->clear();
    try
    {
        ...
        QString result = "Самый короткий путь составляет ";
        result += QString::number(sum);
        ui->Length_route->setText(result);
    }
    catch (...)
    {
        QMessageBox::warning(this, "Тут какая-то ошибка!", "Я не могу вычислить маршрут, помогите х(");
    }
    ui->ScreenDeleteVertex->clear();
    ui->ScreenEdge1->clear();
    ui->ScreenEdge2->clear();
